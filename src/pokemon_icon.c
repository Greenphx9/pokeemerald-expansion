...
// Like CreateMonIcon, but also accepts an isShiny (to support shiny icons)
u8 CreateMonIcon2(u16 species, void (*callback)(struct Sprite *), s16 x, s16 y, u8 subpriority, bool32 isShiny, u32 personality)
{
    u32 paletteNum;
    const u32* palette = GetMonSpritePalFromSpeciesAndPersonality(species > NUM_SPECIES ? SPECIES_NONE : species, isShiny, personality);
    u16 tag = species + (isShiny ? SPECIES_SHINY_TAG : 0) + POKE_ICON_SPECIES_BASE_PAL_TAG;
