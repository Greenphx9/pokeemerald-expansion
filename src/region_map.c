...
void CB2_OpenFlyMap(void)
{
    ...
    case 7:
        ...
        AddTextPrinterParameterized(WIN_FLY_TO_WHERE, FONT_SHORT, gText_FlyToWhere, 0, 1, 0, NULL);
...
static void DrawFlyDestTextWindow(void)
{
    ...
    if (sFlyMap->regionMap.mapSecType > MAPSECTYPE_NONE && sFlyMap->regionMap.mapSecType < NUM_MAPSEC_TYPES)
    {
        ...
        for (i = 0; i < ARRAY_COUNT(sMultiNameFlyDestinations); i++)
        {
            if (sFlyMap->regionMap.mapSecId == sMultiNameFlyDestinations[i].mapSecId)
            {
                if (FlagGet(sMultiNameFlyDestinations[i].flag))
                {
                    ...
                    AddTextPrinterParameterized(WIN_MAPSEC_NAME_TALL, FONT_SHORT, sFlyMap->regionMap.mapSecName, 0, 1, 0, NULL);
                    name = sMultiNameFlyDestinations[i].name[sFlyMap->regionMap.posWithinMapSec];
                    AddTextPrinterParameterized(WIN_MAPSEC_NAME_TALL, FONT_SHORT, name, GetStringRightAlignXOffset(FONT_SHORT, name, 96), 17, 0, NULL);
                    ScheduleBgCopyTilemapToVram(0);
                    sDrawFlyDestTextWindow = TRUE;
                }
                break;
        ...
        if (!namePrinted)
        {
            ...
            AddTextPrinterParameterized(WIN_MAPSEC_NAME, FONT_SHORT, sFlyMap->regionMap.mapSecName, 0, 1, 0, NULL);
            ScheduleBgCopyTilemapToVram(0);
