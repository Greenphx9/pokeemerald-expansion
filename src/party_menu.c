...
static void Task_TryItemUseFormChange(u8 taskId)
{
    ...
    case 3:
        ...
        if (gTasks[taskId].tAnimWait == 0)
        {
            ...
            CreatePartyMonIconSpriteParameterized(targetSpecies, GetMonData(mon, MON_DATA_IS_SHINY), GetMonData(mon, MON_DATA_PERSONALITY, NULL), &sPartyMenuBoxes[gPartyMenu.slotId], 1);
            icon->oam.mosaic = TRUE;
            icon->data[0] = 10;
            icon->data[1] = 1;
            icon->data[2] = taskId;
