...
#define TAG_SWAP_LINE TAG_ROTATING_BALL_GFX
...
static void SpriteCB_FRLG_BagVisualSwitchingPockets(struct Sprite *sprite);
static void SpriteCB_FRLG_ShakeBagSprite(struct Sprite *sprite);
...
void CreateBagSprite(u8 animNum)
{
    ...
    FRLG_SetBagVisualPocketId(animNum);
}
...
void FRLG_SetBagVisualPocketId(u8 animNum)
{
    ...
    sprite->y2 = -5;
    sprite->callback = SpriteCB_FRLG_BagVisualSwitchingPockets;
...
void FRLG_ShakeBagSprite(void)
{
    ...
    if (sprite->affineAnimEnded)
    {
        ...
        sprite->callback = SpriteCB_FRLG_ShakeBagSprite;
    }
}
