...
{
    AddTextPrinterParameterized4(windowId, FONT_SHORT, x, y, 0, 0, sTextColors[colorStructId], speed, text);
}
...
static void AddBerryTagTextToBg0(void)
{
    ...
    PrintTextInBerryTagScreen(WIN_BERRY_TAG, sText_BerryTag, GetStringCenterAlignXOffset(FONT_SHORT, sText_BerryTag, 0x40), 1, 0, 1);
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_SHORT, sText_SizeSlash, 0, 1, TEXT_SKIP_DRAW, NULL);
    if (berry->size != 0)
    {
        u32 inches, fraction;
        ...
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_SHORT, gStringVar4, 0x28, 1, 0, NULL);
    }
    else
    {
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_SHORT, sText_ThreeMarks, 0x28, 1, 0, NULL);
    }
}
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_SHORT, sText_FirmSlash, 0, 0x11, TEXT_SKIP_DRAW, NULL);
    if (berry->firmness != 0)
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_SHORT, sBerryFirmnessStrings[berry->firmness], 0x28, 0x11, 0, NULL);
    else
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_SHORT, sText_ThreeMarks, 0x28, 0x11, 0, NULL);
}

static void PrintBerryDescription1(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_DESC, FONT_SHORT, berry->description1, 0, 1, 0, NULL);
}

static void PrintBerryDescription2(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_DESC, FONT_SHORT, berry->description2, 0, 0x11, 0, NULL);
}
