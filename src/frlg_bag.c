...
const u8 sText_DepositItem[] = _("DEPOSIT ITEM");
...
void InitBagWindows(void)
{
    ...
    LoadUserWindowBorderGfx(0, 0x81, BG_PLTT_ID(12));
...
void BagPrintTextOnWin1CenteredColor0(const u8 * str, u8 unused)
{
    ...
    AddTextPrinterParameterized3(2, FONT_SHORT, x / 2, 1, sTextColors[0], 0, str);
}
...
void BagDrawDepositItemTextBox(void)
{
    ...
    AddTextPrinterParameterized(2, FONT_SMALL, sText_DepositItem, x / 2, 1, 0, NULL);
}
...
{
    CreateYesNoMenuWithCallbacks(taskId, &sWindowTemplates[3], FONT_SHORT, 0, 2, 0x064, 14, ptrs);
}

void BagCreateYesNoMenuTopRight(u8 taskId, const struct YesNoFuncTable * ptrs)
{
    CreateYesNoMenuWithCallbacks(taskId, &sWindowTemplates[4], FONT_SHORT, 0, 2, 0x064, 14, ptrs);
}

void BagPrintMoneyAmount(void)
{
    FRLG_PrintMoneyAmountInMoneyBoxWithBorder(ShowBagWindow(2, 0), 0x081, 0x0C, GetMoney(&gSaveBlock1Ptr->money));
}
