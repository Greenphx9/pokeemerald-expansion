...
struct BagMenuAlloc
{
    ...
    u8 nItems[POCKETS_COUNT + 1];
    u8 maxShowed[POCKETS_COUNT + 1];
    u8 data[4];
};
...
static EWRAM_DATA u8 (*sListMenuItemStrings)[ITEM_NAME_LENGTH + 5] = NULL;
...
static u8 GetDisplayPocketId(u8 prevPocketId);
...
static const u8 *const sPocketNames[] = {
    [POCKET_ITEMS - 1] = gText_Items,
    [POCKET_KEY_ITEMS - 1] = gText_Key_Items,
    [POCKET_POKE_BALLS - 1] = gText_PokeBalls
};
...
static const struct MenuAction sItemMenuContextActions[] = {
    [FRLG_ITEMMENUACTION_USE] = {COMPOUND_STRING("USE"), {.void_u8 = Task_ItemMenuAction_Use}},
    [FRLG_ITEMMENUACTION_TOSS] = {COMPOUND_STRING("TOSS"), {.void_u8 = Task_ItemMenuAction_Toss}},
    [FRLG_ITEMMENUACTION_REGISTER] = {COMPOUND_STRING("REGISTER"), {.void_u8 = Task_ItemMenuAction_ToggleSelect}},
    [FRLG_ITEMMENUACTION_GIVE] = {COMPOUND_STRING("GIVE"), {.void_u8 = Task_ItemMenuAction_Give}},
    [FRLG_ITEMMENUACTION_CANCEL] = {gText_Cancel, {.void_u8 = Task_ItemMenuAction_Cancel}},
    [FRLG_ITEMMENUACTION_BATTLE_USE] = {COMPOUND_STRING("USE"), {.void_u8 = Task_ItemMenuAction_BattleUse}},
    [FRLG_ITEMMENUACTION_CHECK] = {COMPOUND_STRING("CHECK"), {.void_u8 = Task_ItemMenuAction_Use}},
    [FRLG_ITEMMENUACTION_OPEN] = {COMPOUND_STRING("OPEN"), {.void_u8 = Task_ItemMenuAction_Use}},
    [FRLG_ITEMMENUACTION_OPEN_BERRIES] = {COMPOUND_STRING("OPEN"), {.void_u8 = Task_ItemMenuAction_BattleUse}},
    [FRLG_ITEMMENUACTION_WALK] = {COMPOUND_STRING("WALK"), {.void_u8 = Task_ItemMenuAction_Use}},
    [FRLG_ITEMMENUACTION_DESELECT] = {COMPOUND_STRING("DESELECT"), {.void_u8 = Task_ItemMenuAction_ToggleSelect}},
    [FRLG_ITEMMENUACTION_DUMMY] = {gText_Blank, {.void_u8 = NULL}}
};

static const u8 sContextMenuItems_Field[][4] = {
    [POCKET_ITEMS - 1] =
    {
        FRLG_ITEMMENUACTION_USE,
        FRLG_ITEMMENUACTION_GIVE,
        FRLG_ITEMMENUACTION_TOSS,
        FRLG_ITEMMENUACTION_CANCEL
    }, 
    [POCKET_KEY_ITEMS - 1] =
    {
        FRLG_ITEMMENUACTION_USE,
        FRLG_ITEMMENUACTION_REGISTER,
        FRLG_ITEMMENUACTION_CANCEL,
        FRLG_ITEMMENUACTION_DUMMY
    }, 
    [POCKET_POKE_BALLS - 1] =
    {
        FRLG_ITEMMENUACTION_GIVE,
...
static const u8 sContextMenuItems_Open[] = {
    FRLG_ITEMMENUACTION_OPEN,
    FRLG_ITEMMENUACTION_CANCEL
};

static const u8 sContextMenuItems_BattleUse[] = {
    FRLG_ITEMMENUACTION_BATTLE_USE,
    FRLG_ITEMMENUACTION_CANCEL
};
...
static const TaskFunc sItemContextTaskFuncs[] = {
    [FRLG_ITEMMENULOCATION_FIELD]  = Task_ItemContext_FieldOrBattle,
    [FRLG_ITEMMENULOCATION_PARTY]  = Task_ItemContext_FieldGive,
    [FRLG_ITEMMENULOCATION_SHOP]   = Task_ItemContext_Sell,
    [FRLG_ITEMMENULOCATION_ITEMPC] = Task_ItemContext_Deposit,
    [FRLG_ITEMMENULOCATION_PCBOX]  = Task_ItemContext_PcBoxGive,
    [FRLG_ITEMMENULOCATION_BATTLE] = Task_ItemContext_FieldOrBattle,
    [FRLG_ITEMMENULOCATION_OLD_MAN] = NULL
};
...
static const u8 sPocketDisplayOrder[] =
{
    POCKET_ITEMS - 1,
    POCKET_KEY_ITEMS - 1,
    POCKET_POKE_BALLS - 1,
...
void FRLG_GoToBagMenu(u8 location, u8 pocket, MainCallback bagCallback)
{
    u8 i;
    ...
    {
        if (location != FRLG_ITEMMENULOCATION_LAST)
        ...
        if (location == FRLG_ITEMMENULOCATION_ITEMPC)
            sBagMenuDisplay->pocketSwitchMode = 1;
        else if (location == FRLG_ITEMMENULOCATION_OLD_MAN)
...
{
    FRLG_GoToBagMenu(FRLG_ITEMMENULOCATION_FIELD, OPEN_BAG_LAST, CB2_ReturnToFieldWithOpenMenu);
}

void CB2_FRLG_BagMenuFromBattle(void)
{
    FRLG_GoToBagMenu(FRLG_ITEMMENULOCATION_BATTLE, OPEN_BAG_LAST, CB2_SetUpReshowBattleScreenAfterMenu2);
}
...
static bool8 LoadBagMenuGraphics(void)
{
    ...
    case 12:
        ...
        break;
    case 13:
        if (gBagMenuState.location != FRLG_ITEMMENULOCATION_ITEMPC)
            PrintBagPocketName();
        else
...
static u8 CreateBagInputHandlerTask(u8 location)
{
    switch (location)
    {
    case FRLG_ITEMMENULOCATION_OLD_MAN:
        return CreateTask(Task_Bag_OldManTutorial, 0);
    case FRLG_ITEMMENULOCATION_TTVSCR_REGISTER:
        return CreateTask(Task_Bag_TeachyTvRegister, 0);
    case FRLG_ITEMMENULOCATION_TTVSCR_TMS:
        return CreateTask(Task_Bag_TeachyTvTMs, 0);
    case FRLG_ITEMMENULOCATION_TTVSCR_STATUS:
        return CreateTask(Task_Bag_TeachyTvStatus, 0);
    case FRLG_ITEMMENULOCATION_TTVSCR_CATCHING:
...
static void Bag_BuildListMenuTemplate(u8 pocket)
{
    ...
    gMultiuseListMenuTemplate.textNarrowWidth = 100;
}
...
static void bag_menu_print_cursor(u8 y, u8 colorIdx)
{
    if (colorIdx == 0xFF)
    {
        FillWindowPixelRect(0, PIXEL_FILL(0), 1, y, GetMenuCursorDimensionByFont(FONT_SHORT, 0), GetMenuCursorDimensionByFont(FONT_SHORT, 1));
    }
    else
    {
        BagPrintTextOnWindow(0, FONT_SHORT, gText_SelectorArrow2, 1, y, 0, 0, 0, colorIdx);
    }
}
...
static void PrintItemDescriptionOnMessageWindow(s32 itemIndex)
{
    ...
    BagPrintTextOnWindow(1, FONT_SHORT, description, 0, 3, 2, 0, 0, 0);
}
...
void ResetBagCursorPositions(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
    {
        gBagMenuState.itemsAbove[sPocketDisplayOrder[i]] = 0;
        gBagMenuState.cursorPos[sPocketDisplayOrder[i]] = 0;
    }
}
...
static void CalculateInitialCursorPosAndItemsAbove(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
    {
        PocketCalculateInitialCursorPosAndItemsAbove(sPocketDisplayOrder[i]);
    }
}
...
static void UpdatePocketScrollPositions(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
    {
        u8 pocket = sPocketDisplayOrder[i];
        if (gBagMenuState.itemsAbove[pocket] > 3)
        {
            for (j = 0; j <= gBagMenuState.itemsAbove[pocket] - 3; gBagMenuState.itemsAbove[pocket]--, gBagMenuState.cursorPos[pocket]++, j++)
            {
                if (gBagMenuState.cursorPos[pocket] + sBagMenuDisplay->maxShowed[pocket] == sBagMenuDisplay->nItems[pocket] + 1)
                    break;
            }
        }
    }
...
static void All_CalculateNItemsAndMaxShowed(void)
{
    u8 i;
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
        Pocket_CalculateNItemsAndMaxShowed(sPocketDisplayOrder[i]);
}
...
static void Task_BagMenu_HandleInput(u8 taskId)
{
    ...
    case 2:
        ...
        return;
    default:
        if (JOY_NEW(SELECT_BUTTON) && gBagMenuState.location == FRLG_ITEMMENULOCATION_FIELD)
        {
...
{
    s16 *data = gTasks[taskId].data;
    u32 displayPocket = GetDisplayPocketId(gBagMenuState.pocket);
    u32 newPocket = displayPocket + direction;
    tSwitchState = 0;
    tSwitchCounter = 0;
    ...
    ScheduleBgCopyTilemapToVram(1);
    FRLG_SetBagVisualPocketId(sPocketDisplayOrder[newPocket]);
...
static void BeginMovingItemInPocket(u8 taskId, s16 itemIndex)
{
    ...
    FillWindowPixelBuffer(1, PIXEL_FILL(0));
    BagPrintTextOnWindow(1, FONT_SHORT, gStringVar4, 0, 3, 2, 0, 0, 0);
    FRLG_UpdateSwapLinePos(0, ListMenuGetYCoordForPrintingArrowCursor(data[0]));
    FRLG_SetSwapLineInvisibility(FALSE);
    BagDestroyPocketSwitchArrowPair();
    bag_menu_print_cursor_(data[0], 2);
...
static void ExecuteMoveItemInPocket(u8 taskId, u32 itemIndex)
{
    ...
    {
        FRLG_MoveItemSlotInList(gBagPockets[gBagMenuState.pocket].itemSlots, data[1], itemIndex);
        DestroyListMenuTask(data[0], &gBagMenuState.cursorPos[gBagMenuState.pocket], &gBagMenuState.itemsAbove[gBagMenuState.pocket]);
        if (data[1] < itemIndex)
            gBagMenuState.itemsAbove[gBagMenuState.pocket]--;
        Bag_BuildListMenuTemplate(gBagMenuState.pocket);
        data[0] = ListMenuInit(&gMultiuseListMenuTemplate, gBagMenuState.cursorPos[gBagMenuState.pocket], gBagMenuState.itemsAbove[gBagMenuState.pocket]);
        FRLG_SetSwapLineInvisibility(TRUE);
        CreatePocketSwitchArrowPair();
        gTasks[taskId].func = Task_BagMenu_HandleInput;
    }
}
...
static void OpenContextMenu(u8 taskId)
{
    ...
    {
    case FRLG_ITEMMENULOCATION_BATTLE:
    case FRLG_ITEMMENULOCATION_TTVSCR_STATUS:
        if (gSpecialVar_ItemId == ITEM_BERRY_POUCH)
        {
            sContextMenuItemsBuffer[0] = FRLG_ITEMMENUACTION_OPEN_BERRIES;
            sContextMenuItemsBuffer[1] = FRLG_ITEMMENUACTION_CANCEL;
    ...
    default:
        ...
        else
        {
            ...
            case OPEN_BAG_KEYITEMS:
                ...
                sContextMenuItemsBuffer[2] = FRLG_ITEMMENUACTION_CANCEL;
                if (gSaveBlock1Ptr->registeredItem == gSpecialVar_ItemId)
                    sContextMenuItemsBuffer[1] = FRLG_ITEMMENUACTION_DESELECT;
                else
                    sContextMenuItemsBuffer[1] = FRLG_ITEMMENUACTION_REGISTER;
                if (gSpecialVar_ItemId == ITEM_TM_CASE || gSpecialVar_ItemId == ITEM_BERRY_POUCH)
                    sContextMenuItemsBuffer[0] = FRLG_ITEMMENUACTION_OPEN;
                else if (gSpecialVar_ItemId == ITEM_BICYCLE && TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_ACRO_BIKE | PLAYER_AVATAR_FLAG_MACH_BIKE))
                    sContextMenuItemsBuffer[0] = FRLG_ITEMMENUACTION_WALK;
                else
                    sContextMenuItemsBuffer[0] = FRLG_ITEMMENUACTION_USE;
                break;
...
static void Task_FieldItemContextMenuHandleInput(u8 taskId)
{
    ...
    if (IsActiveOverworldLinkBusy() != TRUE)
    {
        ...
        case -1:
            PlaySE(SE_SELECT);
            sItemMenuContextActions[FRLG_ITEMMENUACTION_CANCEL].func.void_u8(taskId);
            break;
        case -2:
            break;
        default:
...
static void Task_ConfirmTossItems(u8 taskId)
{
    ...
    BagPrintTextOnWindow(ShowBagWindow(6, 1), FONT_SHORT, gStringVar4, 0, 2, 1, 0, 0, 1);
...
static void Task_TossItem_Yes(u8 taskId)
{
    ...
    BagPrintTextOnWindow(ShowBagWindow(6, 3), FONT_SHORT, gStringVar4, 0, 2, 1, 0, 0, 1);
...
static void Task_ItemMenuAction_Give(u8 taskId)
{
    ...
    if (!IsWritingMailAllowed(itemId))
        DisplayItemMessageInBag(taskId, FONT_SHORT, gText_CantWriteMail, Task_WaitAButtonAndCloseContextMenu);
...
static void Task_PrintThereIsNoPokemon(u8 taskId)
{
    DisplayItemMessageInBag(taskId, FONT_SHORT, gText_NoPokemon, Task_WaitAButtonAndCloseContextMenu);
}
...
static void Task_PrintItemCantBeHeld(u8 taskId)
{
    ...
    DisplayItemMessageInBag(taskId, FONT_SHORT, gStringVar4, Task_WaitAButtonAndCloseContextMenu);
}
...
static void ReturnToBagMenuFromSubmenu_PCBox(void)
{
    FRLG_GoToBagMenu(FRLG_ITEMMENULOCATION_PCBOX, OPEN_BAG_LAST, CB2_ReturnToPokeStorage);
}
...
static void Task_ItemContext_Sell(u8 taskId)
{
    ...
    else if (GetItemPrice(gSpecialVar_ItemId) == 0)
    {
        ...
        DisplayItemMessageInBag(taskId, FONT_SHORT, gStringVar4, Task_ReturnToBagFromContextMenu);
    }
    else
    {
        data[8] = 1;
        ...
        else
        {
            ...
            DisplayItemMessageInBag(taskId, FONT_SHORT, gStringVar4, Task_InitSaleQuantitySelectInterface);
        }
    }
}
...
{
    s16 *data = gTasks[taskId].data;
    ConvertIntToDecimalStringN(gStringVar3, GetItemPrice(BagGetItemIdByPocketPosition(gBagMenuState.pocket + 1, data[1])) / 2 * data[8], STR_CONV_MODE_LEFT_ALIGN, 6);
    StringExpandPlaceholders(gStringVar4, gText_ICanPayThisMuch_WouldThatBeOkay);
    DisplayItemMessageInBag(taskId, FONT_SHORT, gStringVar4, Task_ShowSellYesNoMenu);
}
...
static void Task_InitSaleQuantitySelectInterface(u8 taskId)
{
    ...
    UpdateSalePriceDisplay(GetItemPrice(BagGetItemIdByPocketPosition(gBagMenuState.pocket + 1, data[1])) / 2 * data[8]);
    BagPrintMoneyAmount();
...
{
    FRLG_PrintMoneyAmount(GetBagWindow(0), 56, 10, amount, 0);
}
...
static void Task_SellItem_Yes(u8 taskId)
{
    ...
    DisplayItemMessageInBag(taskId, FONT_SHORT, gStringVar4, Task_FinalizeSaleToShop);
}
...
static void Task_FinalizeSaleToShop(u8 taskId)
{
    ...
    FRLG_PrintMoneyAmountInMoneyBox(GetBagWindow(2), GetMoney(&gSaveBlock1Ptr->money), 0);
...
static void Task_TryDoItemDeposit(u8 taskId)
{
    ...
    {
        DisplayItemMessageInBag(taskId, FONT_SHORT, gText_NoRoomToStoreItems, Task_WaitAButtonAndCloseContextMenu);
    }
}
...
{
    if (
        gBagMenuState.location == FRLG_ITEMMENULOCATION_OLD_MAN 
     || gBagMenuState.location == FRLG_ITEMMENULOCATION_TTVSCR_CATCHING 
     || gBagMenuState.location == FRLG_ITEMMENULOCATION_TTVSCR_STATUS 
     || gBagMenuState.location == FRLG_ITEMMENULOCATION_TTVSCR_REGISTER 
     || gBagMenuState.location == FRLG_ITEMMENULOCATION_TTVSCR_TMS
    )
        return TRUE;
    return FALSE;
}
...
static void BackUpPlayerBag(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
    {
        sBackupPlayerBag->itemsAbove[sPocketDisplayOrder[i]] = gBagMenuState.itemsAbove[sPocketDisplayOrder[i]];
        sBackupPlayerBag->cursorPos[sPocketDisplayOrder[i]] = gBagMenuState.cursorPos[sPocketDisplayOrder[i]];
    }
...
static void RestorePlayerBag(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
    {
        gBagMenuState.itemsAbove[sPocketDisplayOrder[i]] = sBackupPlayerBag->itemsAbove[sPocketDisplayOrder[i]];
        gBagMenuState.cursorPos[sPocketDisplayOrder[i]] = sBackupPlayerBag->cursorPos[sPocketDisplayOrder[i]];
    }
...
void InitOldManBag(void)
{
    ...
    FRLG_GoToBagMenu(FRLG_ITEMMENULOCATION_OLD_MAN, OPEN_BAG_ITEMS, CB2_SetUpReshowBattleScreenAfterMenu2);
}
...
void InitPokedudeBag(u8 a0)
{
    ...
    case 7:
        ...
        location = FRLG_ITEMMENULOCATION_TTVSCR_STATUS;
        break;
    case 8:
        cb2 = CB2_SetUpReshowBattleScreenAfterMenu2;
        location = FRLG_ITEMMENULOCATION_TTVSCR_CATCHING;
        break;
    }
    FRLG_GoToBagMenu(location, OPEN_BAG_ITEMS, cb2);
}
...
static u8 GetDisplayPocketId(u8 pocketId)
{
    u32 i;
    for (i = 0; i < ARRAY_COUNT(sPocketDisplayOrder); i++)
    {
        if (sPocketDisplayOrder[i] == pocketId)
            return i;
    }
    return 0;
}