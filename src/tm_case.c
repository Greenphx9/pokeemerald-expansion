...
#define IS_HM(itemId) (itemId >= ITEM_HM_CUT && itemId <= ITEM_HM_DIVE)
...
#else
#if TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
static void GreyscaleMonIcons(void);
#endif // TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
static void CreateMonIcons(void);
static void UpdateMonIcons(u16 itemId);
#endif // TM_CASE_MON_ICONS == FALSE
...
static const struct MenuAction sMenuActions[] = {
    [ACTION_USE]  = {gMenuText_Use,  {Action_Use }},
    [ACTION_GIVE] = {gMenuText_Give, {Action_Give}},
...
// defined here so we can config it
#if TM_CASE_MON_ICONS == TRUE
static const u32 sTMCaseMenu_Tilemap[] = INCBIN_U32("graphics/tm_case/menu_icons.bin.lz");
#else
static const u32 sTMCaseMenu_Tilemap[] = INCBIN_U32("graphics/tm_case/menu.bin.lz");
#endif // TM_CASE_MON_ICONS == TRUE
...
static const u16 sTMSpritePaletteOffsetByType[NUMBER_OF_MON_TYPES] = {
    ...
    [TYPE_DARK]     = 0x0f0,
    [TYPE_DRAGON]   = 0x100,
    [TYPE_FAIRY]    = 0x0d0, // uses psychic type disk pal
    [TYPE_STELLAR]  = 0x100, // uses dragon type disk pal
};
...
static void List_ItemPrintFunc(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        if (!IS_HM(BagGetItemIdByPocketPosition(POCKET_TM_HM, itemIndex)))
        {
            ...
            #if TM_CASE_MON_ICONS == TRUE
            TMCase_Print(windowId, FONT_SMALL, gStringVar4, 102, y, 0, 0, TEXT_SKIP_DRAW, COLOR_DARK);
            #else
            TMCase_Print(windowId, FONT_SMALL, gStringVar4, 126, y, 0, 0, TEXT_SKIP_DRAW, COLOR_DARK);
            #endif // TM_CASE_MON_ICONS == TRUE
            #endif // I_REUSABLE_TMS == FALSE
        }
        else
        {
...
static void PrintDescription(s32 itemIndex)
{
    ...
    #if TM_CASE_MON_ICONS == TRUE
    UpdateMonIcons(BagGetItemIdByPocketPosition(POCKET_TM_HM, itemIndex));
    #endif // TM_CASE_MON_ICONS == TRUE
}
...
static void Task_SelectedTMHM_Field(u8 taskId)
{
    ...
    TMCase_Print(WIN_SELECTED_MSG, FONT_SHORT, strbuf, 0, 2, 1, 0, 0, COLOR_DARK);
    Free(strbuf);
...
static void PrintError_ThereIsNoPokemon(u8 taskId)
{
    PrintMessageWithFollowupTask(taskId, FONT_SHORT, gText_ThereIsNoPokemon, Task_WaitButtonAfterErrorPrint);
}
...
static void Task_SelectedTMHM_GiveParty(u8 taskId)
{
    ...
    if (!IS_HM(BagGetItemIdByPocketPosition(POCKET_TM_HM, tSelection)))
    {
...
{
    #if TM_CASE_MON_ICONS == TRUE
    u32 distance = 102 - GetStringWidth(FONT_SHORT, gText_TMCase, 0);
    #else
    u32 distance = 72 - GetStringWidth(FONT_SHORT, gText_TMCase, 0);
    #endif // TM_CASE_MON_ICONS == TRUE
    AddTextPrinterParameterized3(WIN_TITLE, FONT_SHORT, distance / 2, 1, sTextColors[COLOR_LIGHT], 0, gText_TMCase);
}
...
static void HandleCreateYesNoMenu(u8 taskId, const struct YesNoFuncTable *ptrs)
{
    CreateYesNoMenuWithCallbacks(taskId, &sYesNoWindowTemplate, FONT_SHORT, 0, 2, 91, 14, ptrs);
}
...
#else
// palette loading taken from skeli, thanks skeli :)
#if TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
static void GreyscaleMonIcons(void)
{
    u32 i;
	...
	for (i = 0; i < 6; i++)
	{
		//Make a copy of each mon icon palette in the garbage slots usually loaded after the normal 6
        //Adjusted for expansion's 6 pal icons
		memcpy(&gPlttBufferUnfaded[0x100 + (i+7)*16], &gPlttBufferUnfaded[0x100 + i*16], 0x20);
		memcpy(&gPlttBufferFaded[0x100 + (i+7)*16], &gPlttBufferFaded[0x100 + i*16], 0x20);

		//Grayscale that copy
		TintPalette_GrayScale(&gPlttBufferUnfaded[0x100 + (i+7)*16], 16);
		TintPalette_GrayScale(&gPlttBufferFaded[0x100 + (i+7)*16], 16);
	}
}
#endif // TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
...
static void CreateMonIcons(void)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        {
            sTMCaseDynamicResources->monSpriteIds[i] = CreateMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0);
            x += 32;
            if (i == 2) // second row
            {   
                y += 32;
...
static void UpdateMonIcons(u16 itemId)
{
	...
	for (i = 0; i < gPlayerPartyCount; i++)
    {
        struct Sprite *spr = &gSprites[sTMCaseDynamicResources->monSpriteIds[i]];
        struct Pokemon *mon = &gPlayerParty[i];
        u16 species = GetMonData(mon, MON_DATA_SPECIES_OR_EGG);
        u16 move = ItemIdToBattleMoveId(itemId);
        #if TM_CASE_MON_ICON_TYPE == TM_CASE_TRANSLUCENT
        SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_TGT2_ALL);
        SetGpuReg(REG_OFFSET_BLDALPHA, BLDALPHA_BLEND(7, 11));
        #endif // TM_CASE_MON_ICON_TYPE == TM_CASE_TRANSLUCENT
        if (CanLearnTeachableMove(species, move))
        {
            #if TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
            spr->oam.paletteNum = gSpeciesInfo[species].iconPalIndex;
            #else
            spr->oam.objMode = ST_OAM_OBJ_NORMAL;
            #endif // M_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
            #if TM_CASE_DISABLE_ANIM_KNOWS_MOVE == TRUE
            if (MonKnowsMove(mon, move))
                spr->callback = SpriteCallbackDummy;
            else
            #endif // TM_CASE_DISABLE_ANIM_KNOWS_MOVE == TRUE
                spr->callback = SpriteCB_MonIcon;
        }
        else
        {
            #if TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
            spr->oam.paletteNum = gSpeciesInfo[species].iconPalIndex + 7;
            #else
            spr->oam.objMode = ST_OAM_OBJ_BLEND;
            #endif // TM_CASE_MON_ICON_TYPE == TM_CASE_GREYSCALE
			spr->callback = SpriteCallbackDummy;
        }
    }
